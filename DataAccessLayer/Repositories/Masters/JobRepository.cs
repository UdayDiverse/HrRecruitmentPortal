...
namespace DataAccessLayer.Repositories.Masters
{
    public class JobRepository(ApplicationDbContext _context) : IJobRepository
    {
        ...
        {
            return await _context.JobEntity.Include(x => x.Department).Where(x => x.Id == id).FirstOrDefaultAsync();
        }
        ...
        public async Task<JobSearchResponseEntity> SearchJobAsync(JobSearchRequestModel requestModel, string? offset, string count)
        {
            ...
            try
            {
                var query = _context.JobEntity.Include(x => x.Department).AsQueryable();
                ...
                if (requestModel.JobOwnerId.HasValue)
                    query = query.Where(t => t.JobOwnerId == requestModel.JobOwnerId);
                ...
                if (parsedCount == 0)
                {
                    response.Jobs = await query.ToListAsync();
                ...
                {
                    response.Jobs = await query.Skip(parsedOffset).Take(parsedCount).ToListAsync();
